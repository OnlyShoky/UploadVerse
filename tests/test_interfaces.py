...
@patch('video_publisher.get_publisher')
def test_library_upload_video_with_platforms(mock_get_publisher):
...
@patch('video_publisher.get_publisher')
def test_library_upload_video_auto_detect(mock_get_publisher):
...
def test_api_upload_with_file(mock_upload, api_client):
    ...
    # Test with missing file - should return 400
    response = api_client.post('/upload', data={}, content_type='multipart/form-data')
    assert response.status_code == 400
